...
export async function GET(req: NextRequest) {
  ...
  const session = await new EncryptJWT({accessToken:token.access_token}).setProtectedHeader({alg:'dir',enc:'A256GCM'}).setIssuedAt().setExpirationTime('7d').encrypt(secret);
  const response = NextResponse.redirect(new URL('/', req.url));
  response.cookies.set('gh_session', session, {httpOnly:true,secure:process.env.NODE_ENV==='production',sameSite:'lax',maxAge:60*60*24*7,path:'/'});
